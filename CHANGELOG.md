...
- ASKING FOR A TRUEHD TRACK BY THE WRONG CODEC NAME KEPT FOUR PER CENT OF THE AUDIO. A Blu-ray stores TrueHD with an AC-3 compatibility core in front of it and the lossless frames interleaved behind, and this program lists such a track as A_AC3. A line asking for it as A_MLP, which is what TrueHD is called everywhere else here, skipped the core frames and took most of the lossless data with them, then reported success: a demux produced 1,841,388 bytes where A_AC3 produces 12,177,210, and a mux produced 2,719,608 where A_AC3 produces 17,611,652. The size is not the dangerous part. The short file decodes 1.54 seconds of a 39.873 second track before it hits invalid data, while every label on it is right, a probe reporting TrueHD at 48 kHz, 8 channels and 24 bit, so checking the result the way anyone checks a demux shows nothing wrong at all; and the muxed version is worse, because this program reads its own output back as a healthy TrueHD track. It is refused now, naming the codec that works, which is the same advice the down-to-ac3 warning already carried for this arrangement and which until now only appeared if that option happened to be present. The test is the disc arrangement itself, an AC-3 sync word at the very start of the track with the first lossless frame somewhere behind it, so a standalone TrueHD stream begins with its own frame and never matches. Refused rather than warned about, because a warning leaves behind a file that looks finished, and refused rather than quietly read as A_AC3, because that would hand back a different file than the line asks for.
- A TRACK NUMBER THE FILE DOES NOT HAVE PRODUCED AN EMPTY FILE AND REPORTED SUCCESS. Nothing ever matched the number, so no data was produced, the output was zero bytes, and "Demux complete" was printed after it with exit code zero. Measured the same way on a DVD program stream, on a Matroska file and on a transport stream, so it is not particular to one container. The ordinary way to arrive at it is to write a meta against the track numbers of the file a source was made from, because a remuxed or demuxed file does not keep them. It is refused now, before the track is registered, and the message lists the track numbers the file does hold and says why a copied one will not match. The track list is read once when the container is opened and kept, since building it can mean re-reading the file for the program map and this runs once per track line. Where a container does not report a list the check is skipped, so nothing that worked before is refused; a program stream reports every identifier in its range as a hint rather than what is actually present, so that format is only covered for numbers outside the range, which is partial and still better than silence because it produces no false refusals.
